...
class DynPricesFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            valid = await self._test_credentials(
                user_input[CONF_ENTSOE_TOKEN], user_input[CONF_ENTSOE_AREA]
            )
            if valid:
                return self.async_create_entry( title=user_input[CONF_NAME], data=user_input )
            else:
                self._errors["base"] = "auth"
                _LOGGER.error("cannot authenticate auth")
    ...
    async def _test_credentials(self, token, area):
        ...
        try:
            ...
            _LOGGER.exception("credentials failed")
            pass
        return False
...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> dict[str, Any]:
    ...
    hub = EntsoeHub(data["TOKEN"], data["AREA"] )
    ...
    return {"title": "Dynamic Grid Price Solar"}
